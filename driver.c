...
for(int nbAttempts = 0; nbAttempts < max_iters; nbAttempts++){
    input1 = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, buffer_size * sizeof(int), input_buffer_1 , &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to create first input buffer! %d\n", err);
         exit(EXIT_FAILURE);
     }
    input2 = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, buffer_size * sizeof(int), input_buffer_2 , &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to create second input buffer! %d\n", err);
         exit(EXIT_FAILURE);
     }  
    output = clCreateBuffer(ocl->context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, buffer_size * sizeof(int), output_buffer , &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to create output buffer! %d\n", err);
         exit(EXIT_FAILURE);
     }  
    status_buf = clCreateBuffer(ocl->context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(status), &status , &err);
     if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(ocl->command_queue, input1, CL_TRUE, 0, buffer_size * sizeof(int), input_buffer_1, 0, NULL, NULL);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to write first input buffer into device! %d\n", err);
         exit(EXIT_FAILURE);
     }  
 
    err= clEnqueueWriteBuffer(ocl->command_queue, input2, CL_TRUE, 0, buffer_size * sizeof(int), input_buffer_2, 0, NULL, NULL);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to write second input buffer into device! %d\n", err);
         exit(EXIT_FAILURE);
     }  
    ...
    err = clSetKernelArg(ocl->kernel, 0, sizeof(input1), &input1);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set first arg (input buffer 1) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 1, sizeof(input2), &input2);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set second arg (input buffer 2) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 2, sizeof(output), &output);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set third arg (output buffer) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 3, sizeof(status), &status);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set fourth arg (status) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 4, sizeof(w1), &w1);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set fifth arg (w1) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 5, sizeof(w2), &w2);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set sixth arg (w2) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clSetKernelArg(ocl->kernel, 6, sizeof(buffer_size), &buffer_size);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set seventh arg (buffer_size) in kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
...
*/
    err = clEnqueueNDRangeKernel(ocl->command_queue, ocl->kernel, 1, NULL, &global, &local, 0, NULL, NULL);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to enqueue a command to execute a kernel on device! %d\n", err); //INCORRECT KERNEL ARGS
         exit(EXIT_FAILURE);
     }
    ...
    printf("right before clFinish\n");
    err = clFinish(ocl->command_queue);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to block until all commands are completed! %d\n", err);
         exit(EXIT_FAILURE);
     }

    // Check the status

    //read status buffer from kernel into status
    err = clEnqueueReadBuffer(ocl->command_queue, status_buf, CL_TRUE, 0, sizeof(status), &status, 0, NULL, NULL);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to read out the status from the kernel! %d\n", err);
         exit(EXIT_FAILURE);
     }
    // When the status is 0 (success), read back the results from the device to verify the output
    if(status[0] == 0){
	    err = clEnqueueReadBuffer(ocl->command_queue, output, CL_TRUE, 0, buffer_size * sizeof(int), output_buffer, 0, NULL, NULL);
	     if (err != CL_SUCCESS) {
		 fprintf(stderr,"Error: Failed to read out the output from the device! %d\n", err);
		 exit(EXIT_FAILURE);
	     }	   	    
    }
    ...
    err = clReleaseMemObject(input1);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to release memory for input buffer 1! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clReleaseMemObject(input2);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to release memory for input buffer 2! %d\n", err);
         exit(EXIT_FAILURE);
     }
    err = clReleaseMemObject(output);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to release memory for output buffer! %d\n", err);
         exit(EXIT_FAILURE);
     }
    break;
}
